...
final class ClientTests: XCTestCase {
    ...
    func test_gpt4Completion() async throws {
        let messages: [Chat.Message] = [
            .init(
                ...
                content: [
                    .text(
                        "You are a fairytale storyteller. Create a fairytale about the subject in the next message."
                    )
                ]),
            .init(role: .user, content: [.text("a happy wolf in the forrest")]),
        ]
    ...
    func test_createChat() async throws {
        let completion = try await client.chats.create(
            ...
            messages: [
                .init(role: .user, content: [.text("Write a haiki")])
            ]
        )
    ...
    func test_createEmbedding() async throws {
        let embedding = try await client.embeddings.create(
            input: "he food was delicious and the waiter...")
    ...
    func test_createModeration() async throws {
        let moderation = try await client.moderations.createModeration(
